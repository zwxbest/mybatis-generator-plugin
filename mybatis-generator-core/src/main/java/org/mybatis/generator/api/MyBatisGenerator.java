...
public class MyBatisGenerator {
    ...
    public void generate(ProgressCallback callback, Set<String> contextIds,
        ...
        //todo:填充generatedJavaFiles和generatedXmlFiles
        for (Context context : contextsToRun) {
    ...
    private void writeGeneratedJavaFile(GeneratedJavaFile gjf, ProgressCallback callback)
        ...
        try {
            ...
            if (targetFile.exists()) {
                //todo:先不处理Example,有内部类有些麻烦.而且目前没有改动Example的需求
//                if (shellCallback.isMergeSupported()) {
                if (gjf.isMergeable()&&!targetFile.getName().endsWith("Example.java")) {
                    //todo
                    source=shellCallback.mergeJavaFile(gjf.getCompilationUnit(),targetFile);
